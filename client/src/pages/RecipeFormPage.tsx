...
export default function RecipeFormPage() {
  ...
  if (isEdit && isLoadingRecipe) return <div>Loading...</div>;

  return (
    <PageLayout>
      <div className="max-w-4xl mx-auto">
        <h1 className="text-3xl font-display font-bold mb-8">{isEdit ? "Edit Recipe" : "Create New Recipe"}</h1>

        {!isEdit && (
          <div className="bg-card rounded-2xl p-6 border border-primary/20 shadow-lg shadow-primary/5 mb-8">
            <div className="flex items-center gap-3 mb-4 text-primary">
              <Sparkles className="w-5 h-5" />
              <h2 className="font-bold">Magic Import with AI</h2>
            </div>
            <div className="flex gap-2">
              ...
              >
                {importRecipe.isPending ? <Loader2 className="w-4 h-4 animate-spin" /> : "Import"}
              </button>
            </div>
          </div>
        )}
        ...
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
          ...
          <section className="bg-card rounded-2xl p-6 border border-border space-y-6">
            <h3 className="font-display font-bold text-lg border-b border-border pb-2">Basic Info</h3>
          ...
          <section className="bg-card rounded-2xl p-6 border border-border space-y-4">
            <div className="flex items-center justify-between border-b border-border pb-2">
              <h3 className="font-display font-bold text-lg text-primary">Ingredients</h3>
              <button type="button" onClick={() => appendIng({ name: "", quantity: "", unit: "pc" })} className="text-sm text-primary font-bold hover:underline flex items-center gap-1">
            ...
            <div className="space-y-3">
              {ingredients.map((field, index) => (
                <div key={field.id} className="flex gap-3">
                  <input {...form.register(`ingredients.${index}.quantity`)} placeholder="Qty" className="w-20 px-3 py-2 rounded-lg border border-border" />
                  <input {...form.register(`ingredients.${index}.unit`)} placeholder="Unit" className="w-20 px-3 py-2 rounded-lg border border-border" />
                  <input {...form.register(`ingredients.${index}.name`)} placeholder="Name" className="flex-1 px-3 py-2 rounded-lg border border-border" />
          ...
          <section className="bg-card rounded-2xl p-6 border border-border space-y-4">
            <div className="flex items-center justify-between border-b border-border pb-2">
              <h3 className="font-display font-bold text-lg text-orange-600">Spices & Seasonings</h3>
              <button type="button" onClick={() => appendSpice({ name: "", quantity: "" })} className="text-sm text-orange-600 font-bold hover:underline flex items-center gap-1">
                <Plus className="w-4 h-4" /> Add Spice
              </button>
            </div>
            ...
            <div className="space-y-3">
              {spices.map((field, index) => (
                <div key={field.id} className="flex gap-3">
                  <input {...form.register(`spices.${index}.quantity`)} placeholder="Qty (opt)" className="w-24 px-3 py-2 rounded-lg border border-border" />
                  <input {...form.register(`spices.${index}.name`)} placeholder="Spice Name" className="flex-1 px-3 py-2 rounded-lg border border-border" />
          ...
          <section className="bg-card rounded-2xl p-6 border border-border space-y-4">
             <div className="flex items-center justify-between border-b border-border pb-2">
              <h3 className="font-display font-bold text-lg">Instructions</h3>
